...
router.post('/:id/report', authenticateToken, checkNotApprovedForUser, (req, res) => {
  ...
  db.query(sql, [experimentId, userId, tools_used || null, procedure_text || null, result || null], (err) => {
    if (err) {
      console.error('Error saving experiment report:', err);
      return res.status(500).json({ message: 'Failed to save experiment report' });
    }
...
router.get('/:id/report', authenticateToken, (req, res) => {
  ...
  db.query(sql, params, (err, rows) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Failed to load experiment report' });
    }
...
const handleApprovedFiles = (req, res) => {
  ...
  db.query(sql, (err, rows) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Failed to fetch approved experiment files' });
    }
    return res.json(rows || []);
  });
};
