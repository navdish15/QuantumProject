...
router.get('/profile', (req, res) => {
  ...
  db.query('SELECT id, name, email, role, avatar, phone, prefs FROM users WHERE id = ? LIMIT 1', [userId], (err, rows) => {
    if (err) {
    ...
    checkPassword()
      .then((ok) => {
        ...
        bcrypt.hash(newPassword, saltRounds, (hashErr, hash) => {
          ...
          db.query('UPDATE users SET password = ? WHERE id = ?', [hash, userId], (uErr) => {
            if (uErr) {
              console.error('PUT /admin/change-password update error:', uErr);
...
router.post('/avatar', avatarUpload, (req, res) => {
  ...
  db.query('UPDATE users SET avatar = ? WHERE id = ?', [publicPath, userId], (err) => {
    if (err) {
...
router.put('/notifications/prefs', (req, res) => {
  ...
  db.query('UPDATE users SET prefs = ? WHERE id = ?', [JSON.stringify(prefs), userId], (err) => {
    if (err) {
...
router.get('/notifications/prefs', (req, res) => {
  const userId = getUserId(req);
  db.query('SELECT prefs FROM users WHERE id = ? LIMIT 1', [userId], (err, rows) => {
    if (err) {
